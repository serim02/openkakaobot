...
/* 상점 아이템 */
let shopItems = {
  "닉네임 지정권": { price: 1000, effect: "원하는 닉네임으로 하루동안 변경 또는 지정권(수위 지키기)" },
...
function saveRoomData(room) {
  try {
    ...
    // 저장 실패 시 에러 처리
    console.log("데이터 저장 실패: " + e.message);
  }
}
...
function saveGlobalData() {
  try {
    ...
    console.log("전역 데이터 저장 실패: " + e.message);
  }
}
...
function loadGlobalData() {
  try {
    ...
    console.log("전역 데이터 로드 실패: " + e.message);
  }
}
...
function initializeUser(room, username) {
  ...
  if (!data.users[username]) {
    data.users[username] = createDefaultUserData();
  }
}
...
function startMVPTimer(room, replier) {
  ...
  // 이미 오늘 8시가 지났으면 내일로 넘김
  if (now > target) {
    target.setDate(target.getDate() + 1);
  }

  let msUntil8PM = target - now;
...
function response(room, msg, sender, isGroupChat, replier, imageDB, packageName) {
  ...
  if (!msg.startsWith("!")) {
    ...
    // 데이터 저장 (잭팟 터졌거나, 레벨업했거나, 매 20번째 채팅마다)
    if (jackpotTriggered || levelUpTriggered || data.users[sender].chatCount % 20 === 0) {
      saveRoomData(room);
    }
  ...
  // === 구매 ===
    ...
    let itemName = msg.substring(4).trim(); // split 대신 substring 사용
    if (!itemName) {
  ...
  // === 관리자 명령어 ===
    ...
    // 잭팟 설정 초기화
      ...
      globalJackpotConfig = {
        ...
        cooldown: 10             // 기본 쿨다운 5분
      };
      replier.reply("🎰 잭팟 설정이 기본값으로 초기화되었습니다!\n" +
                    "확률: 1% | 보상: 50~500P | 쿨다운: 10분");
      saveGlobalData();
    }
    
    // 관리자 자신의 데이터 초기화
    else if (msg === "!초기화") {
      data.users[sender] = createDefaultUserData();
    ...
    // 특정 사용자 데이터 초기화
      ...
      data.users[targetUser] = createDefaultUserData();
